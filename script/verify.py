...
class Reporter:
    ...
    def print(self, *, verbose: bool = False) -> None:
        ...
        for tgt in sorted(by_target.keys()):
            items = sorted(
                by_target[tgt], key=lambda x: (sev_order.get(x.severity, 9), x.check)
            )
            print(f"\n== {tgt} ==")
            for it in items:
...
class SSH:
    ...
    def run(
        self, host: str, remote_cmd: str, *, sudo: bool = False, timeout: int = 30
    ) -> Tuple[int, str, str]:
        if sudo:
...
def detect_os_family(ssh: SSH, target: str, rep: Reporter) -> Optional[Tuple[str, str]]:
    ...
    if rc != 0 or not out:
        rep.warn(
            target,
            "os-detect",
            f"Unable to read /etc/os-release (rc={rc}): {err or 'no output'}",
        )
        return None
...
def check_file_perms(
    ...
    rc, out, err = ssh.run(
        target, f"stat -c '%a %U:%G %n' {shell_escape(path)} 2>/dev/null"
    )
    if rc != 0 or not out:
...
def check_tls_host_material(
    ssh: SSH, target: str, cfg: Dict[str, Any], fqdn: str, rep: Reporter
) -> None:
    ...
    if out.strip() == "OK":
        ...
        rep.warn(
            target,
            "tls-ca-chain",
            f"Missing: {chain} (some services may require chain)",
        )

    # SAN contains fqdn
    rc, out, err = ssh.run(
        target,
        f"openssl x509 -in {shell_escape(crt)} -noout -text 2>/dev/null | sed -n '/Subject Alternative Name/,+2p'",
    )
    if rc != 0 or not out:
    ...
    rc, _, _ = ssh.run(
        target, f"openssl x509 -in {shell_escape(crt)} -noout -checkend 0"
    )
    if rc != 0:
    ...
    rc, _, _ = ssh.run(
        target, f"openssl x509 -in {shell_escape(crt)} -noout -checkend {warn_seconds}"
    )
    if rc != 0:
...
def check_systemd_timers(ssh: SSH, target: str, rep: Reporter) -> None:
    ...
    for t in timers:
        rc, out, err = ssh.run(
            target, f"systemctl is-enabled {shell_escape(t)} 2>/dev/null || true"
        )
        if out.strip() == "enabled":
            ...
            rep.warn(
                target,
                "timer-enabled",
                f"{t}: unknown ({out.strip() or err or 'no output'})",
            )
    ...
    for k, v in directives.items():
        ...
        if rc == 0 and out:
            ...
            rep.warn(
                target,
                "sshd-config",
                f"Did not find '{k} {v}' in /etc/ssh/sshd_config (may be in included config)",
            )
...
def verify_host(
    ...
    ip = host.get("ip") or host.get("primary_ip") or ""
    target = ip or fqdn
    ...
    if is_truthy(ssh_cfg.get("manage_host_cert", True)):
        ...
        rep.warn(
            target,
            "ssh-host-cert",
            "SSH host cert management disabled for this host in config",
        )
    ...
    if ilo and is_truthy(ilo.get("present", False)):
        ...
        rep.passed(
            target,
            "ilo",
            f"present (strategy={strategy}, gen={ilo.get('generation', '?')}, fqdn={ilo.get('fqdn', '?')})",
        )
...
def verify_router(router: Dict[str, Any], rep: Reporter) -> None:
    ...
    if cp.returncode != 0 and "BEGIN CERTIFICATE" not in (cp.stdout or ""):
        rep.fail(
            target,
            "router-tls",
            f"unreachable or TLS failure (rc={cp.returncode}): {cp.stderr.strip()}",
        )
        return

    # Extract first cert and run openssl x509 checks locally
    m = re.search(
        r"-----BEGIN CERTIFICATE-----.*?-----END CERTIFICATE-----", cp.stdout, re.S
    )
    if not m:
    ...
    if cp3.returncode != 0:
        rep.warn(
            target, "router-expiry", f"expires within {warn_days} days (or invalid)"
        )
    else:
